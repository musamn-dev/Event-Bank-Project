...
export default class Deposit extends LightningElement {
    @track description;
    @track amount;
    @track eventId;
    @track balance;
    ...
    handleDeposit() {
        createTransaction({ eventId: this.eventId, amount: this.amount, description: this.description, type: 'Deposit' })
            .then(() => {
                this.showToast('Success', 'Deposit successful', 'success');
                this.loadAccountBalance();
            })
            .catch(error => {
                this.showToast('Error', error.body.message, 'error');
